...
Artifacts are written to `./dist` in the directory where the command runs. If
the command is run from a directory already named `dist`, that directory is
used directly rather than creating `dist/dist`.
